...
try {
	...
	$search = isset($params->search[0])
						? (!$params->searchtype) 
							? " AND
									(
										UPPER(de.nome) LIKE UPPER(:query)
									)
								"
							: " AND
									(
	...
	$searchDelegado = (isset($params->search[0]) && !$params->searchtype) ? $search : null;
	...
	$stmt = $oConexao->prepare(
		'SELECT
			...
			(SELECT 
				COUNT(id) 
			FROM localdelegadoitem 
			WHERE idlocaldelegado=lc.id
			) as qtd,
			(select group_concat(
				concat(" ", de.nome )
				) as delegado
				from localdelegadoitem del
				inner join delegado de on de.id = del.iddelegado
				where del.idlocaldelegado = lc.id '. $searchDelegado .'
			) as delegados
	...
	foreach ($results as $value) {
		if($value['delegados']!=null){
			$value['delegados'] = explode(",", $value['delegados']);
			$result[] = $value;
			$limit++;
		}
	}
