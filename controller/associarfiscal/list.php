...
try {
	...
	$search = isset($params->search[0]) 
						? (!$params->searchtype) 
							? " AND
									(
										UPPER(fi.nome) LIKE UPPER(:query)
									)
								"
							: " AND
									(
										UPPER(lc.local) LIKE UPPER(:query)
									)
								"
						: null;

	$searchFiscal = (isset($params->search[0]) && !$params->searchtype) ? $search : null;
	...
	$stmt = $oConexao->prepare(
		'SELECT
			...
			(SELECT 
				COUNT(id) 
			FROM localfiscalitem 
			WHERE idlocalfiscal=lc.id
			) as qtd,
			(select group_concat(
				concat(" ", fi.nome )
				) as fiscal
				from localfiscalitem fcl
				inner join fiscal fi on fi.id = fcl.idfiscal
				where fcl.idlocalfiscal = lc.id '. $searchFiscal .'
			) as fiscais
		FROM localfiscal lc
		INNER JOIN cidade c 
		ON lc.idcidade = c.id
		INNER JOIN localvotacao lv
		ON lc.local = lv.local
		INNER JOIN bairro b
		ON lv.bairro = b.id
	...
	foreach ($results as $value) {
		if($value['fiscais']!=null){
			$value['fiscais'] = explode(",", $value['fiscais']);
			$result[] = $value;
			$limit++;
		}
	}
