...
try {
	...
	$search = isset($params->search[0])
						? (!$params->searchtype) 
							? " AND
									(
										UPPER(cc.nome) LIKE UPPER(:query)
									)
								"
							: " AND
									(
	...
	$searchCoordenador = (isset($params->search[0]) && !$params->searchtype) ? $search : null;
	...
	$stmt = $oConexao->prepare(
		'SELECT
			...
			(SELECT 
				COUNT(id) 
			FROM localcoordenadoritem 
			WHERE idlocalcoordenador=lc.id
			) as qtd,
			(select group_concat(
				concat(" ", cc.nome )
				) as coordenador
				from localcoordenadoritem lcc
				inner join coordenador cc on cc.id = lcc.idcoordenador
				where lcc.idlocalcoordenador = lc.id '. $searchCoordenador .'
			) as coordenadores
	...
	foreach ($results as $value) {
		if($value['coordenadores']!=null){
			$value['coordenadores'] = explode(",", $value['coordenadores']);
			$result[] = $value;
			$limit++;
		}
	}
